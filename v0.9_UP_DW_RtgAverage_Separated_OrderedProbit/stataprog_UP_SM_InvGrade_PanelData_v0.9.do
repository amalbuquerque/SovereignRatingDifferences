...
import delimited using UP_SM_InvGrade_PanelData_WithAvgRating, delimiters(";") clear
...
esttab using UP_SM_gross_debt_InvGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_net_debt_InvGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_budgetbal_InvGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_structbal_InvGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
