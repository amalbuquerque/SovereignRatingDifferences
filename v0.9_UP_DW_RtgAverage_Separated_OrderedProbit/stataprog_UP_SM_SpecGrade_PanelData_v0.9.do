...
cd v0.9_UP_DW_RtgAverage_Separated_OrderedProbit
import delimited using UP_SM_SpecGrade_PanelData_WithAvgRating, delimiters(";") clear
...
esttab using UP_SM_gross_debt_SpecGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_net_debt_SpecGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_budgetbal_SpecGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
...
* showing in a nice format and storing it in a .csv file to import to Word
estout m1 m2 m3 m4, starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant)
esttab using UP_SM_structbal_SpecGrade_results.csv, replace star(* 0.10 ** 0.05 *** 0.01)
