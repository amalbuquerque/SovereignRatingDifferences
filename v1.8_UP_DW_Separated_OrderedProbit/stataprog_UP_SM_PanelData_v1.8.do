...
import delimited using UP_SM_PanelData, delimiters(";") clear
...
foreach s in grossdebt netdebt budgetbal structbal {
    ...
    * the default way is to place each model on each column
    esttab using UP_SM_`s'_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)

    mat list r(coefs)
    mat rename r(coefs) foo`s'
    mat list foo`s'
    * to get the results transposed
    esttab matrix(foo`s', transpose) using UP_SM_`s'_results_transpose.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
