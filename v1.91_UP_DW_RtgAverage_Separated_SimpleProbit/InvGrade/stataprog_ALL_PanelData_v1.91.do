...
foreach s in grossdebt netdebt budgetbal structbal {
    ...
    * esttab using UP_MF_`s'_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
    esttab using `pair'_`s'_InvGrade_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)

    mat list r(coefs)
    mat rename r(coefs) foo`pair'`s'
    mat list foo`pair'`s'
    * to get the results transposed
    esttab matrix(foo`pair'`s', transpose) using `pair'_`s'_InvGrade_results_transpose.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
