...
foreach s in grossdebt netdebt budgetbal structbal {
    ...
    * to get the results transposed
    esttab matrix(foo`pair'`s', transpose) using `pair'_`s'_results_InvGrade_transpose.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
