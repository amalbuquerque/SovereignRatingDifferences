...
foreach s in grossdebt {
    ...
    *esttab using r, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
    ...
    /* 2016/09/17 00:01:35, AA: fitstat doesn't work with panel data
     * we have to do it manually
    quietly fitstat
    estadd fitstat, r2_mf : *
    */
    ...
    * the default way is to place each model on each column
    esttab using UP_MF_`s'_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) scalars(r2_mf N) legend label varlabels(_cons Const)
