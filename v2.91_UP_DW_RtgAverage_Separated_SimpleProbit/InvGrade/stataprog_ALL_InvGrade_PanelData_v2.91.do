...
cd v2.91_UP_DW_RtgAverage_Separated_SimpleProbit
cd InvGrade
...
import delimited using `pair'_InvGrade_PanelData_WithAvgRating, delimiters(";") clear
...
foreach s in grossdebt netdebt budgetbal structbal {
    ...
    foreach y in 1 2 5 10 {
        ...
        * 2016/09/17 07:17:45, AA: running the simple probit,
        * constant-only model to get the log-likelihood of it (ll_0)
        quietly xtprobit $ylist if e(sample)==1, vce(robust)
        local my_ll0 = e(ll)
    ...
    * esttab using UP_MF_`s'_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) pr2 legend label varlabels(_cons Const)
    esttab using `pair'_`s'_InvGrade_results_wide.csv, mtitles replace star(* 0.10 ** 0.05 *** 0.01) scalars(r2_mf N) legend label varlabels(_cons Const)
